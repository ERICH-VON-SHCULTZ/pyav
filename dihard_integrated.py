...
##       self.device = original_model.device
        ...
        if model == 0 or model == 3 or model == 4:
            ...
            # Model 7: Early Fusion (CoAttention) ONLY
            # Uses the same CoAttentionEncoder as Model 6, but applies it BEFORE LSTM (early fusion)
            self.second_input = torch.nn.Sequential(
                torch.nn.Linear(esize, 60),
                torch.nn.ReLU(),
                torch.nn.Linear(60, 60),
            ...
            # CoAttentionEncoder for early fusion
            # Audio dimension is 60 (from sincnet output), video dimension is 60 (from second_input)
            self.co_attention_encoder = CoAttentionEncoder(
                audio_embed_dim=60,  # sincnet outputs 60 features
                video_embed_dim=60,   # second_input outputs 60 features
                num_heads=4,          # Must divide both embed dims
                num_layers=2,
                dropout=0.1
            )

            # Merge layer to combine enhanced audio and video embeddings
            # Input: concatenated [60 + 60] = 120
            # Output: 60 (to match expected input for LSTM)
            self.merge = torch.nn.Sequential(
                torch.nn.Linear(120, 60),
                torch.nn.ReLU()
            )
            ...
            # Model 8: Early Fusion (Linear) + Late Fusion (Attention)
            
            # --- Early Fusion Stage (Linear) ---
            ...
            # Linear layers replacing CoAttention for Early Fusion
            self.early_audio_linear = nn.Sequential(
                nn.Linear(60, 60),
                nn.ReLU(),
                nn.Dropout(0.1)
            )
            self.early_video_linear = nn.Sequential(
                nn.Linear(60, 60),
                nn.ReLU(),
                nn.Dropout(0.1)
            )
            ...
            self.early_merge = torch.nn.Sequential(
                torch.nn.Linear(120, 60),  # 60+60=120 -> 60 (to LSTM)
                torch.nn.ReLU())
            
            # --- Late Fusion Stage (Attention) ---
            self.second_input = torch.nn.Sequential(
                torch.nn.Linear(esize, 60),
                torch.nn.ReLU(),
                torch.nn.Linear(60, 60),
            ...
            self.co_attention_encoder = CoAttentionEncoder(
                audio_embed_dim=128,  # LSTM output
                video_embed_dim=60,   # Late video projection
                num_heads=4,
                num_layers=2,
                dropout=0.1
            )
            ...
            self.merge = torch.nn.Sequential(
                torch.nn.Linear(188, 128), # 128+60 -> 128
                torch.nn.ReLU(),
                torch.nn.Linear(128, 128),
                torch.nn.ReLU()
            )
    ...
    def forward(self, audio, npy) -> torch.Tensor:
        ...
        # Prepare variables for flow control
        outputs_npy_interp = None 

        # --- MODEL 7 & 8 EARLY FUSION LOGIC ---
        if model == 7 or model == 8:
            sincnet_frames = outputs_sincnet.size()[2]
            
            # Select correct input projection
            if model == 7:
                # Model 7 uses second_input for early fusion
                outputs_npy = self.second_input(npy)
            else: # model == 8
                # Model 8 uses early_second_input for early fusion
                outputs_npy = self.early_second_input(npy)

            outputs_npy_interp = F.interpolate(rearrange(outputs_npy, "batch feature frame -> batch frame feature"), sincnet_frames)

            # Rearrange to [batch, frames, features]
            audio_features = rearrange(outputs_sincnet, "batch feature frame -> batch frame feature")
            video_features = rearrange(outputs_npy_interp, "batch feature frame -> batch frame feature")
            
            # Apply Early Fusion Logic
            if model == 7:
                # Model 7: Early Fusion with CoAttention
                enhanced_audio, enhanced_video = self.co_attention_encoder(audio_features, video_features)
                concat_features = torch.cat((enhanced_audio, enhanced_video), 2)  # 60+60=120
                merged_features = self.merge(concat_features) # 120->60
            else: # model == 8
                # Model 8: Early Fusion with Linear Layers
                audio_processed = self.early_audio_linear(audio_features)
                video_processed = self.early_video_linear(video_features)
                concat_features = torch.cat((audio_processed, video_processed), 2) # 60+60=120
                merged_features = self.early_merge(concat_features) # 120->60

            # Prepare for LSTM
            outputs = rearrange(merged_features, "batch frame feature -> batch feature frame")

        # --- STANDARD LATE FUSION PREP (Models 0-6) ---
        if not (model == 7 or model == 8):
            outputs_npy = self.second_input(npy)
            sincnet_frames = outputs_sincnet.size()[2]
            outputs_npy_interp = F.interpolate(rearrange(outputs_npy, "batch feature frame -> batch frame feature"), sincnet_frames)
        ...
        # Note: For Model 7 and 8, if justaudio is True, we should also revert to sincnet outputs
        if (model == 7 or model == 8) and justaudio:
             outputs = outputs_sincnet
        ...
        # --- MODEL 8 LATE FUSION (Attention) ---
        if model == 8:
            # outputs is now [batch, frames, 128 (LSTM output)]
            
            # Use second_input for late fusion video processing
            outputs_npy_late = self.second_input(npy) 
            lstm_frames = outputs.size()[1]
            
            # Interpolate to match LSTM frame count
