...
def process_files_in_directory(input_dir, output_dir):
    ...
    offset_file = open("/scratch/map22-share/AVA-AVD/AVA-AVD/dataset/offsets", "w") 
    ...
    for filename in os.listdir(input_dir):
        ...
        if os.path.isfile(input_filepath):
            ...
            for line in content:
                ...
                min_time = min(min_time, float(linesplit[3]))
            print(f"{Path(input_filepath).stem} {min_time}", file=offset_file)
            
            for line in content:            
                linesplit = line.strip().split()
                linesplit[3] = f"{float(linesplit[3]) - min_time:.6f}"
...
if __name__ == "__main__":
    ...
    destination_directory = "/scratch/map22-share/AVA-AVD/AVA-AVD/dataset/rttms_new2"
